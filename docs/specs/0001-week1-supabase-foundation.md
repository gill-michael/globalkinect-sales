# Spec 0001 — Week 1: Supabase Foundation, Auth Scaffold, Ingestion Spine
...
**Changes from v1:**
- Database is Supabase (new project, isolated from existing Supabase projects), not Cloud SQL
- Clerk confirmed for auth
- Connection management via Supabase's standard tooling
...
In scope:
- Stand up new isolated Supabase project for the sales engine (separate from existing Supabase projects)
...
- Rewire `scripts/vibe_prospecting_scan.py` to dual-write (Supabase + Notion)
- One-shot backfill: import existing HubSpot Contacts/Companies into Supabase
...
- Verify end-to-end: a fresh Vibe scan produces deduplicated rows in Supabase; a test JWT validates against the FastAPI middleware

Out of scope (defer to week 2 or later):
- HubSpot bidirectional sync (write-back from Supabase to HubSpot) — week 2
...
- Use of Supabase Auth, PostgREST auto-API, Realtime, or Storage features — out of scope this week. Supabase used as Postgres-only this week. Other features may be adopted later via separate ADRs.
...
- [ ] **New Supabase project** created at `supabase.com`, named `gk-sales-prod`, region: closest to London (e.g. `eu-west-2` London or `eu-west-1` Ireland — Supabase's exact region naming may vary)
- [ ] **This is a fresh project, not the existing sales/dashboard Supabase project.** Sales canonical state must not collide with the existing Supabase mirror tables or the Lovable dashboard's auth project. If clarification is needed about which existing Supabase project is which, run a quick audit before creating the new one.
- [ ] Supabase project's database password set and stored securely
- [ ] Database schema `public` will be used (Supabase default); no need to create another schema
- [ ] Connection details accessible:
  - `SALES_SUPABASE_URL` — the project URL (e.g. `https://abcdefgh.supabase.co`)
  - `SALES_SUPABASE_PUBLISHABLE_KEY` — publishable key (Supabase's renamed anon key — safe for clients)
  - `SALES_DATABASE_URL` — the direct Postgres connection string (`postgresql://postgres:<password>@db.<project>.supabase.co:5432/postgres`)
- [ ] All four values added to `.env`, with `.env.example` updated to document them (placeholder values only)
- [ ] Branch created: `git checkout -b week-1-supabase-foundation`

If any of these aren't done, do them first and confirm before starting code work.

**Naming clarity:** Throughout this spec, "Supabase" refers to the new sales project, not to any pre-existing Supabase project on the account. Where the existing platform/dashboard Supabase is referenced, it is named explicitly.
...
│   └── README.md                      how to run migrations against the sales Supabase project
├── sales_services/                    NEW
│   ├── __init__.py
...
├── sales_ingestion/                   NEW
│   ├── __init__.py
│   ├── vibe_to_supabase.py            Vibe → Supabase write helper (used by modified scan script)
│   ├── hubspot_backfill.py            one-shot HubSpot → Supabase import
│   └── reports_backfill.py            one-shot leads/Reports/ folder walk → Supabase
├── sales_api/                         NEW (skeleton only this week — full API is week 2/4)
│   ├── __init__.py
│   ├── main.py                        FastAPI app with /healthz and /me endpoints only
│   └── auth_middleware.py             JWT validation middleware
└── scripts/
    └── vibe_prospecting_scan.py       MODIFIED — now dual-writes Supabase + Notion
```
...
**On Supabase usage this week:** All database access this week goes through the standard Postgres connection string (`SALES_DATABASE_URL`) using `psycopg2`, exactly as it would for any Postgres database. Supabase-specific features (PostgREST auto-API, Realtime subscriptions, Supabase Auth, Storage) are **not used this week**. Treat Supabase as managed Postgres only. Future ADRs may adopt PostgREST or Realtime if a clear use-case emerges, but week-1 scope is Postgres-only.
...
- One function `get_connection()` that reads `SALES_DATABASE_URL` from env and returns a `psycopg.connect()` connection (psycopg v3).
- Use `psycopg` v3 (already pinned at `psycopg[binary]==3.2.10` in requirements) — do not add new deps. Mirror the API in existing `migrations/run_migrations.py` (`import psycopg`, `psycopg.connect(...)`, `with conn.transaction():`).
- All connections opened with `autocommit=False` by default.
- The connection string points at the new sales Supabase project's Postgres endpoint. Supabase uses standard Postgres protocol — no special handling needed.
- Helper also exposes `get_supabase_client()` for future use, returning a `supabase-py` client initialised with `SALES_SUPABASE_URL` and `SALES_SUPABASE_PUBLISHABLE_KEY`. **Not used this week** but stub the function so it's available when needed without re-touching this module. The `supabase` package is already pinned at `2.28.3` in `requirements.txt`; no change to dependencies. If server-admin Supabase operations are required later (e.g. bypassing RLS in a backend job), add `SALES_SUPABASE_SERVICE_KEY` via a future ADR — out of scope for week 1.

**`sales_db/runner.py`:**
- Reads all `*.sql` files in `sales_db/migrations/` in lexicographic order.
- Maintains a `schema_migrations` table (auto-created if absent) tracking applied migrations by filename.
...
**Acceptance:**
- `python -m sales_db.runner` against the empty Supabase sales project creates `schema_migrations` and exits cleanly.
- Running it twice in a row is a no-op the second time.
- A `psycopg` (v3) connection to `SALES_DATABASE_URL` succeeds with the credentials from `.env`.

> **Amended 2026-05-03** after spec/codebase conflict surfaced during initial implementation: psycopg v3 (not psycopg2), `schema_migrations` (not `_schema_migrations`), `supabase` already pinned at 2.28.3.
...
**Note on Supabase's `auth.users` table:** Supabase ships with its own `auth.users` table for Supabase Auth. We are **not** using Supabase Auth, and the table we are creating lives in the `public` schema. The two will not collide. Our `users` table is the canonical record; `auth.users` (if Supabase populates it) is ignored.

**Acceptance:**
- After running, `\dt public.*` in the Supabase SQL editor (or via psql) shows `users` and `pipelines`.
...
- `contacts.status` `NOT NULL DEFAULT 'new'`, with CHECK constraint on the eight defined statuses.
...
- Inserting a Contact without specifying `status` defaults it to `'new'`.
...
`scripts/vibe_prospecting_scan.py` — modify to **add** Supabase writes alongside existing Notion writes. **Do not remove the Notion writes this week** — dual-write protects against rollback.

Modification points:

1. After each Explorium prospect is normalised into the existing dict shape, **also** call new function `write_to_sales_supabase(prospect_dict)` from a new module `sales_ingestion/vibe_to_supabase.py`.

2. `write_to_sales_supabase()` does:
   - Open Supabase Postgres connection via `sales_db.connection.get_connection()`
...
- The sales Supabase project's `contacts` table grows by exactly the number of non-duplicates.
- Notion Lead Intake DB still receives all prospects (existing behaviour unchanged).
- Re-running the same scan adds zero new Supabase rows (dedup works).
- The existing platform/dashboard Supabase project is untouched.
...
3. Writes Companies and Contacts into the sales Supabase, with `source='hubspot_import'`, `hubspot_id` populated, `source_metadata` containing the full HubSpot response.
4. Uses dedup library — if a Contact already exists in the sales Supabase (e.g., from an earlier Vibe scan), updates the `hubspot_id` field on the existing row but does not overwrite other data.
...
- A `--limit 10` run creates ≤10 new sales-Supabase `contacts` rows, each with `hubspot_id` set.
...
1. Truncate all tables in the sales Supabase project (or recreate the project from scratch if cleaner).
2. Run all migrations: `python -m sales_db.runner`. Confirms 6 migrations applied.
3. Run HubSpot backfill with `--limit 50`. Confirm 50 new Supabase rows.
...
All week-1 decisions are locked:

| # | Question | Decision |
|---|---|---|
| 1 | Canonical email | `michael@globalkinect.ae` |
| 2 | Database provider | Supabase (new project, isolated from existing Supabase projects) |
| 3 | Auth provider | Clerk |
| 4 | Supabase region | Closest to London (eu-west-1 or eu-west-2 — Supabase's exact region naming applies) |
| 5 | Supabase Auth, PostgREST, Realtime, Storage usage | Not used this week. Postgres connection only. |
...
- New isolated sales Supabase project exists, separate from existing Supabase projects on the account
- All 8 tables created with correct constraints, indexes, and FKs
- Two pipelines seeded with all stages
- Michael (admin) seeded as a `users` row at `michael@globalkinect.ae`
- Dedup library finds existing Contacts by email, LinkedIn, or name+company
- Vibe scans dual-write to sales Supabase + Notion, with dedup applied
- Existing HubSpot Contacts/Companies imported into sales Supabase
- Existing `leads/Reports/` content reflected as `contacts` + `assets` rows in sales Supabase (folder untouched)
...
- All code lands on `week-1-supabase-foundation` branch. PR to main at end of week, after the end-to-end verification passes.
- Verification scripts: write to a temp `.py` file outside the repo (e.g. `%TEMP%/verify_taskN.py`), execute with `PYTHONPATH=<repo-root> python <file>`, delete after. Inline heredoc Python via bash on this Windows + Git Bash setup mishandles em-dashes and other unicode. The temp-file pattern used in Task 4 is canonical going forward.
