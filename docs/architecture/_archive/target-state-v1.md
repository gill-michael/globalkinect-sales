> **SUPERSEDED on 2026-05-03** by `target-state-v2.md` in this directory. Retained for architectural-decision audit trail.
...
first_name        text
last_name         text
full_name         text                  -- generated/computed
email             text
mobile            text
linkedin_url      text
job_title         text
...
id              uuid PK
contact_id      uuid FK contacts(id)
deal_id         uuid FK deals(id)       -- nullable: pre-deal activities exist
type            text                    -- 'email_sent' | 'email_opened' | 'email_replied' | 'email_bounced' | 'call_made' | 'call_connected' | 'call_voicemail' | 'linkedin_sent' | 'linkedin_replied' | 'meeting_booked' | 'note' | 'stage_change'
direction       text                    -- 'outbound' | 'inbound' | 'internal'
channel         text                    -- 'email' | 'linkedin' | 'phone' | 'meeting' | 'system'
payload         jsonb                   -- channel-specific (email subject, call duration, etc.)
performed_by    text                    -- SDR username | 'system' | 'instantly' | 'claude'
...
company_id      uuid FK companies(id)
account_owner   text
strategic_value text                    -- 'tier_1' | 'tier_2' | 'tier_3'
notes           text
```
...
- **Multi-tenant rep ownership rules.** Single SDR (Michael) for now. When more reps come, owner/team logic gets added.
...
**Operators editing in Notion:** changes are *not* synced back. If the operator wants to change canonical state, they do it in the dashboard or HubSpot. Notion is read-only for state, edit-capable for notes.
**Why keep Notion at all in the target state:** Michael is comfortable in Notion, the agents already know how to write to it, retiring it entirely in month 1 is unnecessary churn. Decision on full Notion retirement is a month-3+ question.
...
### Week 1 — Postgres foundation + ingestion

**Goal:** Postgres exists, one ingestion path works, dedup works.

- Stand up new Cloud SQL Postgres instance (small tier, ~$30/month). Isolated from platform.
- Schema migrations: all tables in section 4. Run via existing migration runner pattern.
...
**Goal:** SDR works the day from the new dashboard. Operator Console retires.

- FastAPI backend layer over Postgres — the dashboard's data path.
- Next.js dashboard with three views:
  - **Today** — list of due actions, ranked, filterable by motion. One-click into Contact view.
  - **Contact** — research, asset previews, sequence state, activity history, action buttons.
  - **Deals** — pipeline view, drag-to-advance, motion-filterable.
- Dashboard auth: shared-secret middleware on FastAPI for the one-month window.
- SDR runs through full day: ingest → enrich → research → sequence → approve → send → reply → next action — all in dashboard.
- Operator Console marked deprecated, port reused by FastAPI.

**Success criteria:**
- A new lead from Vibe ingestion appears in Today view within 1 hour, fully enriched, with assets ready, with a recommended next action.
- SDR completes a full action (e.g., approve email → push to Instantly → mark sent) without leaving the dashboard.
- A reply to a sent email appears in Today view as a new task within 5 minutes of receipt.
...
- Notion still exists as a read-only review surface for those who want it.
...
- Multi-rep ownership and assignment rules. Month 2 or later.
- Production-grade auth on the API layer. Month 2.
...
| 4 | Notion stops being canonical; becomes one-way review surface | Notion's API and rate limits can't support a custom dashboard; canonical state needs SQL. |
...
These are things I need explicit decisions on, not assumptions:

1. **Cloud SQL Postgres tier.** Smallest tier is fine for month 1 (~$30/month). Confirm budget approval, or specify constraints.
2. **HubSpot Deal pipelines.** I've proposed two pipelines (end-buyer, EOR). HubSpot allows up to 100 per portfolio in their tier system. Confirm the two-pipeline split, or push back.
3. **Sequence templates.** I've named two (`direct_outbound_5touch`, `eor_partnership_long`) but not designed the EOR one in detail. The 5-touch direct sequence is the one already produced. The EOR sequence needs design — probably 6-8 touches over 3 months, mostly LinkedIn + email, with a different tone. Flag for a separate spec.
4. **Notion's long-term role.** Month 1 is "keep it as one-way mirror." Month 3+ decision: full retirement, or permanent role as the human-edit surface for unstructured content (notes, account briefs)? Defer.
5. **Authentication on the dashboard.** Shared-secret in month 1 is fine for solo Michael. Confirm that. Plan for JWT/SSO when more SDRs join — that's a separate spec.
6. **Backup and recovery.** Cloud SQL handles automated backups by default. Acceptable for the one-month window. Production-grade DR is a later concern.
7. **Cost ceiling on AI.** Perplexity at $0.10-0.15/lead, Claude at $0.05-0.10/lead. Eager generation on every lead is ~$500/month at current Vibe volumes. Tiered (top decile only) is ~$50-80/month. Confirm tiered approach, or specify ceiling.

These need answers before week 1 starts. Specifically items 1, 2, 5, and 7 are blocking.
